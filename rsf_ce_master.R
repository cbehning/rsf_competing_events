...
# To run train_predict.R, data_generation.R has to be run first. Parameters set in the data generation
# will be used in train_predict
...
# These G hat data were included for illustration propuse. As this requires to install a version of ranger, that
...
# one simulation run of setup1.
# Please make sure to change the parameters in line 38 onward, in case setup2 or more simulation runs
# were created and need further preprocessing.
# The resulting data  data generated by 1000 repeats of the simulation 
# are provided as summary statistics in this repository.
# If all 45000 simulation runs per setting were run, please use
# source(preprocessing_results.R)
# to preprocess the results outputtet from the random forest.
# WARNING: As this takes a lot of resources to run, please consider loading the results below instead.

#"<setup>_status_freq.rda" contains status frequencies of all runs.
load("setup1_status_freq.rda")
...
load("setup1_results_data.rda")

# For reproducing results of setup2, please run:
#load("setup2_status_freq.rda")
#load("setup2_importance.rda")
#load("setup2_results_data.rda")

######
## D) Application on synthetic data
######
# Instead of the original figures and tables, the application can be run on a synthetic
# data set.
# Therefore, please follow the instructions in
# source(application.R)
# To run ranger, please make sure the ranger version is compiled on your machine, as described in
# the file train_predict.R.
# If the random forest 
# 
...
############################################
##### Part 2: Reproduce Figure & Tables ####
############################################

# The evaluation.R created all figures and tables of the  results as presented in the manuscript
